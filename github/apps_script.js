...
function togglePersonalCheckColumnsActiveSheet() {
  const sheet = SpreadsheetApp.getActiveSheet();
  const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];

  const firstPersonalCol = PERSONAL_MEMBERS
    .map(name => headers.indexOf(name) + 1)
    .find(col => col > 0);

  if (!firstPersonalCol) return;

  const show = sheet.isColumnHiddenByUser(firstPersonalCol);
  setPersonalColumnsVisibilityForSheet(sheet, show);
}

function hidePersonalCheckColumns() {
  setPersonalColumnsVisibility(false);
}

function showPersonalCheckColumns() {
  setPersonalColumnsVisibility(true);
}

function setPersonalColumnsVisibility(show) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
...
function setPersonalColumnsVisibilityForSheet(sheet, show) {
  const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  ...
  PERSONAL_MEMBERS.forEach(name => {
    ...
    if (show) sheet.showColumns(col);
    else sheet.hideColumns(col);
  });
}

function getTargetSheets() {
  return ["出先予定", "車検管理", "会議予定", "行事予定", "作業状況", "工事予定", "電話履歴", "一覧スケジュール"];
}
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem("このシートの個人確認を表示/隠す", "togglePersonalCheckColumnsActiveSheet")
...
function createCheckGroup() {
  ...
  getTargetSheets().forEach(name => {
    ...
    const start =
      headers.indexOf("佐藤確認") + 1;

    const end =
      headers.indexOf("伊藤確認") + 1;
