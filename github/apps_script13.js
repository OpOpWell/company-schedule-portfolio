/**
 * 社内共有管理 v8.3 GitHub公開用サンプル名版（設定シート・免許資格・備品修理・帳簿PDF追加）
 *
 * - 担当者15人と既読確認者3人を分離
 * - 既読確認列は山田・高橋・鈴木の3列のみ
 * - 担当プルダウンは山田を先頭にした15人
...
function processCompletedVehicleInspections() {
  ...
  for (let row = 2; row <= lastRow; row++) {
    ...
    const oldInsuranceDate = insuranceCol > 0 ? sheet.getRange(row, insuranceCol).getValue() : "";
    const nextInsuranceDate = nextInsuranceCol > 0 ? sheet.getRange(row, nextInsuranceCol).getValue() : "";
    ...
    const rowObj = {
      ...
      "新保険期限": nextInsuranceDate || oldInsuranceDate,
    ...
    // 次回保険期限が入力されている場合だけ、保険期限へ反映する
    if (insuranceCol > 0 && nextInsuranceCol > 0 && nextInsuranceDate) {
      sheet.getRange(row, insuranceCol).setValue(nextInsuranceDate);
      sheet.getRange(row, nextInsuranceCol).clearContent();
    }
