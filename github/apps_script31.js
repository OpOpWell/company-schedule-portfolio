...
const SYSTEM_VERSION = "v9.7.21-vehicle-onedit-fixed";
...
const DATE_HEADERS = [
  "日付", "投稿日", "登録日", "購入日", "契約日", "開始日", "終了日", "車検期限", "車検予定日", "取得日", "更新期限", "修理依頼日", "返却予定日", "作成日時", "対応日", "移動日", "期限"
];

const TIME_HEADERS = ["時間", "開始時刻", "終了時刻"];
const VEHICLE_NUMBER_HEADERS = ["車番", "折り返し電話番号"];
const NUMBER_HEADERS = ["契約金額", "金額"];
  ...
  ui.createMenu("社内管理")
    ...
    .addSubMenu(
      ui.createMenu("保守用：個別修復")
        ...
        .addItem("選択行を車検予定日で履歴追加＋期限更新", "applyVehicleInspectionUpdateForActiveRow")
        .addItem("選択行の車検履歴を手動追加", "addVehicleInspectionHistoryForActiveRow")
        .addItem("運転免許シートのズレ調整", "alignLicenseSheetLayout")
        .addItem("免許・資格・備品だけコンパクト表示", "compactLicenseQualificationEquipmentSheets")
    )
    .addSubMenu(
...
function repairConstructionScheduleDropdownAndColors() {
  ...
  toast_("工事予定の未契約プルダウン・通知完了・カラーを修復しました");
}
...
function refreshInputSheets() {
  ...
  toast_("入力シートだけ設定反映を実行しました（プルダウン・通知・色を更新）");
}
...
function getSheetHeaders_() {
  ...
  return {
    ...
    "工事予定": ["工事名", "現場", "依頼主", "連絡先", "契約日", "契約金額", "開始日", "終了日", "状態", "担当", "通知", "電話対応", READ_HEADER, ...personal, "備考", CALENDAR_ID_HEADER, "工事ID"],
...
function getStatusListForSheet_(sheetName) {
  const map = {
    ...
    "工事予定": [CLEAR_LABEL, "予定", "未契約", "着工前", "施工中", "完了", "延期", "中止"],
...
function getDropdownListForHeader_(sheetName, header) {
  ...
  const qualificationCategoryList = [CLEAR_LABEL, "資格", "技能講習", "特別教育", "安全教育"];
...
function ensureSettingsSheet_() {
  ...
  if (current[0] !== "設定種別" || current[1] !== "値") {
    ...
    DEFAULT_COMPANY_VEHICLES.forEach(name => rows.push(["社用車", name, "社用車プルダウン用"]));
...
function ensureDefaultSettingRows_(settingType, defaults, note) {
  ...
  if (lastRow >= 2) {
    ...
    values.forEach(r => {
      ...
      if (type === settingType && value) existingValues[value] = true;
    });
  }

  // 既存のユーザー編集は残し、不足している初期値だけ追記する。
  const rows = [];
  (defaults || []).forEach(v => {
    const value = String(v || "").trim();
    if (!value) return;
    if (existingValues[value]) return;
    rows.push([settingType, value, note]);
  });
...
function applyDataValidationByHeaders_(sheet, headers) {
  ...
  headers.forEach((header, index) => {
    ...
    if (NUMBER_HEADERS.includes(header)) setNumberColumn(sheet, col);
...
function setNumberColumn(sheet, col) {
  const rowCount = getApplyRowCount_(sheet);
...
function getConstructionNoticeText_(row) {
  ...
  if (["施工中", "完了", "更新済", "確認済"].includes(status)) return "完了";
...
function getPeriodNoticeText_(startValue, endValue, status) {
  ...
  if (["完了", "更新済", "確認済", "返却済", "精算済"].includes(s)) return "完了";
...
function handleVehicleInspectionStatusEdit_(e) {
  ...
  if (!["車検状態", "車検予定日", "車検期限"].includes(editedHeader)) return false;

  const result = processVehicleInspectionUpdatedRow_(sheet, row);

  // 履歴追加の成否に関係なく、更新済は必ず空欄へ戻す。
  sheet.getRange(row, statusCol).clearContent();
  ...
  if (result.added) {
    ...
    toast_("【車検更新済処理】状態は空欄に戻しました。履歴は未追加です（" + (result.reason || "条件不一致") + "）");
  }
...
function applyVehicleInspectionUpdateForActiveRow() {
  ...
  if (statusCol > 0) sheet.getRange(row, statusCol).clearContent();
  ...
  if (result.added) {
    toast_("【車検予定日方式】車検履歴へ追加し、車検期限を更新しました");
  } else {
    toast_("【車検予定日方式】履歴は未追加です（" + (result.reason || "条件不一致") + "）");
  }
}
...
function processVehicleInspectionUpdatedRow_(vehicleSheet, row) {
  ...
  const currentDue = toDateOnly_(obj["車検期限"]);
  const plannedDue = toDateOnly_(obj["車検予定日"]);
  ...
  // 車検期限を新期限へ更新し、
  // 車検予定日と車検状態を空欄へ戻す。
  if (currentDue && plannedDue && currentDue.getTime() !== plannedDue.getTime()) {
    const added = appendVehicleInspectionHistoryFromRow_(vehicleSheet, row, currentDue, plannedDue, "更新済");

    if (dueCol > 0) {
      vehicleSheet.getRange(row, dueCol).setValue(plannedDue);
    }

    if (plannedCol > 0) {
      vehicleSheet.getRange(row, plannedCol).clearContent();
    }

    return { added: added, reason: added ? "" : "同じ履歴が既にある可能性があります" };
  }

  // 車検期限を直接変更していた場合の保険処理。
  const remembered = getRememberedVehicleInspectionDueChange_(vehicleSheet, row);
  if (remembered && remembered.oldDue) {
    const oldDue = remembered.oldDue;
    const newDue = plannedDue || currentDue || remembered.newDue;

    if (!newDue) {
      return { added: false, reason: "新しい車検期限が空欄です" };
    }

    const added = appendVehicleInspectionHistoryFromRow_(vehicleSheet, row, oldDue, newDue, "更新済");

    if (plannedDue && dueCol > 0) {
      vehicleSheet.getRange(row, dueCol).setValue(plannedDue);
  ...
  if (currentDue && plannedDue && currentDue.getTime() === plannedDue.getTime()) {
    return { added: false, reason: "車検期限と車検予定日が同じため旧期限が分かりません" };
  }

  if (!plannedDue) {
    return { added: false, reason: "車検予定日が空欄です" };
  }

  if (!currentDue) {
    return { added: false, reason: "現在の車検期限が空欄です" };
  }

  return { added: false, reason: "履歴追加条件に一致しません" };
}
...
function sortVehicleInspectionSheetByDue_() {
  ...
  try {
    sheet.getRange(2, 1, sheet.getLastRow() - 1, headers.length)
      .sort([{ column: dueCol, ascending: true }]);
  } catch (e) {
...
function appendVehicleInspectionHistoryFromRow_(vehicleSheet, row, oldDue, newDue, note) {
  if (!vehicleSheet || vehicleSheet.getName() !== "車検管理" || row <= 1) return false;
...
function normalizeVehicleInspectionUpdatedRow_(sheet, row) {
  ...
  if (result.added) {
    toast_("【車検予定日方式】車検履歴へ追加し、車検期限更新＋状態空欄戻しを行いました");
  } else if (result.reason) {
    toast_("【車検予定日方式】状態は空欄に戻しました。履歴は未追加です（" + result.reason + "）");
  } else {
    toast_("【車検予定日方式】状態は空欄に戻しました。履歴は未追加です");
  }
}
...
function appendGptMenuSpecDiagnostics_(rows) {
  ...
  rows.push([
    ...
    "車検期限変更時に自動追加",
    "OK",
    "車検状態を更新済にした時点で、車検履歴追加・車検期限更新・車検状態空欄戻しを同時に行います。車検予定日が入力されている場合は、車検期限を旧期限、車検予定日を新期限として履歴化します。",
  ...
  rows.push([
    ...
    "未来日に更新済なら状態だけ空欄へ戻す",
    "OK",
    "車検状態を更新済にした時点で、同じ行をスキャンし、車検期限=旧期限・車検予定日=新期限として履歴追加・期限更新・状態空欄戻しを同時に行います。自動で動かない場合は『選択行を車検予定日で履歴追加＋期限更新』で同じ処理を手動実行できます。",
  ...
  const menuFunctions = [
    ...
    ["選択行の車検履歴を手動追加", "addVehicleInspectionHistoryForActiveRow", "保守用：個別修復"],
...
function appendGptAppSheetDiagnostics_(ss, rows) {
  ...
  rows.push(["AppSheet", "数値列", "Number/Price型候補", "情報", "工事予定: 契約金額、日報レシート管理: 金額", "AppSheet Regenerate後にNumber/Price型を確認"]);
...
function standardizeAllInputSheetsReadable() {
  ...
  toast_("全入力シートを標準表示に整えました（" + count + "シート）" + (skipped.length ? " / 未作成: " + skipped.join(", ") : ""));
}
...
function standardizeActiveSheetReadable() {
  ...
  ensureV9714MinorLayoutUpdates_();
  applyDisplayLayoutForSheet_(sheet, "standard");
  toast_("今のシートを標準表示に戻しました: " + sheet.getName());
}
...
function compactActiveSheet() {
  ...
  toast_("今のシートだけコンパクト表示にしました: " + sheet.getName());
}
...
function applyLicenseEquipmentDisplayLayout_(sheet, mode) {
  ...
  headers.forEach((header, index) => {
    ...
    if (sheetName === "運転免許管理") {
      ...
      else if (["取得日", "更新期限"].includes(header)) width = compact ? 106 : 120;
...
function formatSheetByName_(sheetName) {
  ...
  const widthMap = {
    ...
    "契約日": 110, "契約金額": 110, "連絡先": 130, "折り返し電話番号": 150, "対応完了日時": 150, "修理依頼者": 130, "修理業者": 160, "PDFリンク": 260, "フィードバックID": 180
  };
