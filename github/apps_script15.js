/**
 * 社内共有管理 v9.5 GitHub公開用サンプル名版（既読グループ修正・サンプルデータ・機能チェック・軽量化版）
 *
 * - 担当者15人と既読確認者3人を分離
 * - 既読確認列は山田・高橋・鈴木の3列のみ
 * - 担当プルダウンは山田を先頭にした15人
 ...
 * - 車検管理に次回保険期限を追加し、車検更新完了処理で保険期限も更新可能に変更
 * - 日報・車検・長文入力シートのセル幅調整を追加し、更新後に列幅が戻りにくいよう調整
 * - 自動出力シート（一覧スケジュール・要確認一覧など）は入力規則を付けず、集約時の入力規則エラーを防止
 * - 車検更新処理を保険期限あり/なし両対応に変更
 * - 他部門・経理共有向けの一覧帳簿PDFをA4横PDF出力に強化
 * - 車検更新処理を一括読み書きへ寄せ、処理回数を削減
 * - 備品修理管理は、返却済/完了/中止になるまで過去一覧へ移動しない安全仕様を明確化

 * v9.6追加
 * - 社内管理メニューを「初期セットアップ・日常運用・表示保守・月次処理・テスト用」に整理
 * - 本番用初期セットアップを追加
 * - テスト環境セットアップを追加
 * - 後付け実行関数を個別実行だけでなくセットアップから一括実行できるように整理
 * - サンプルデータ追加はテスト専用メニューへ分離
 * - 再現手順を「コード貼付→保存→再読み込み→本番用初期セットアップ」で説明できる構成へ変更
 * - AppSheet本番移行前の安定化として、主要入力シートにID列を追加
 * - 日報にPDFリンク列を追加し、選択行の日報PDF作成時にURLを保存
 * - 電話履歴は「状態」と「電話対応」の重複を避け、対応状況は電話対応に一本化
 * - 電話履歴の旧「状態」列は、既存データ保護のため「対応メモ」として残す
 * - 500行固定を廃止し、日付形式・入力規則・色付けをシート最大行数まで適用
 * - 入力がシート末尾に近づいたら行を自動追加し、入力規則を再適用
 * - 帳簿PDF履歴・お知らせ・要確認一覧・過去一覧も月別グループ化対象に追加
 * - 作成日時・更新日・移動日を日付列候補に追加
 * - 既読グループは「既読」〜個人確認列のみ対象にし、PDFリンク・ID列を巻き込まない
 * - テスト用サンプルデータ追加機能を復活
 * - 過去一覧移動ルールの確認用チェック関数を追加
 * - 設定反映時に自動で全体更新しないよう軽量化
 */
...
const MIN_DATA_ROWS_FOR_VALIDATION = 1000;

// 入力が最終行付近まで到達した時に追加する行数。
const AUTO_EXTEND_ROWS_BUFFER = 200;

// 最終行から何行以内に入力されたら自動で行を追加するか。
const AUTO_EXTEND_TRIGGER_MARGIN = 50;
...
function getStatusListForSheet_(sheetName) {
  const map = {
    ...
    "一覧スケジュール": [CLEAR_LABEL, "予定", "移動中", "着工前", "施工中", "進行中", "未対応", "対応中", "折返し", "要確認", "使用中", "返却待ち", "返却済", "未依頼", "依頼済", "修理中", "予約済", "実施済", "有効", "更新予定", "更新済", "期限切れ", "失効", "下書き", "提出済", "確認済", "差戻し", "未着手", "完了", "延期", "中止", "予約重複"],
    "要確認一覧": [CLEAR_LABEL, "予定", "移動中", "着工前", "施工中", "進行中", "未対応", "対応中", "折返し", "要確認", "使用中", "返却待ち", "返却済", "未依頼", "依頼済", "修理中", "予約済", "実施済", "有効", "更新予定", "更新済", "期限切れ", "失効", "下書き", "提出済", "確認済", "差戻し", "未着手", "完了", "延期", "中止", "予約重複", "重要"],
...
/**
 * 運用ガイドシートを作成する。
 * GitHub公開・社内引継ぎ用に「どのメニューをいつ押すか」をシート内に残す。
 */
function createOperationGuideSheet() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName("運用ガイド");
  if (!sheet) sheet = ss.insertSheet("運用ガイド");

  sheet.clear();
  sheet.clearFormats();

  const rows = [
    ["区分", "実行メニュー", "使うタイミング", "注意"],
    ["初期", "本番用初期セットアップを実行", "本番コピー・新規構築時", "サンプルデータは入りません"],
    ["初期", "テスト環境セットアップを実行", "テストシート作成時", "本番では実行しない"],
    ["日常", "全体更新（軽量）", "通常更新", "普段はこれだけでよい"],
    ["日常", "一覧帳簿PDFを作成", "PDF帳簿を出したい時", "帳簿出力設定で対象を選ぶ"],
    ["保守", "個人確認グループを作り直す", "担当者・既読確認者変更後", "既読列は表示、個人列だけ折りたたみ"],
    ["保守", "日報PDFリンク列を修復", "PDFリンクにTRUE/FALSEが出た時", "一度だけでよい"],
    ["月次", "月次メンテナンス", "月1回", "過去移動を含むため本番では確認後に実行"],
    ["テスト", "過去移動ルールをチェック", "移行前確認", "実データは移動しない判定用"],
    ["非表示", "裏方シートを非表示", "運用画面を整理したい時", "設定・SQL・集計系を隠す"]
  ];
  ...
  ui.createMenu("社内管理")
    .addSubMenu(
      ui.createMenu("1. 初期セットアップ")
        .addItem("本番用初期セットアップを実行", "initialSetupForProduction")
        ...
        .addItem("運用ガイドを作成", "createOperationGuideSheet")
    )
    .addSubMenu(
...
function initialSetupForProduction() {
  ...
  try {
    SpreadsheetApp.getActiveSpreadsheet().toast("本番用初期セットアップを開始します");

    createCompanySheets();
    setupSettingsSheet();
    applySettingsToSheets();
    setupLedgerOutputSettingsSheet();
    createSqlSupportSheets();
    createOperationGuideSheet();
    ...
    SpreadsheetApp.getActiveSpreadsheet().toast("本番用初期セットアップが完了しました");
  } finally {
    lock.releaseLock();
  }
}
 ...
 * 日報に「自分の現場以外で、会社全体として動いている主な現場」を補足する目的で使います。
 */
...
function getDefaultLedgerOutputSettings_() {
  return [
    ...
    { enabled: false, title: "電話履歴一覧", sheetName: "電話履歴", outputHeaders: ["日時", "相手", "内容", "電話対応", "担当", "状態", "通知", "備考"], maxRows: 40, note: "電話対応確認用" },
...
function addSampleDataForTest() {
  ...
  appendObjectsToSheet(ss.getSheetByName("出先予定"), [
    {"日付": today, "行き先": "市役所サンプル", "用件": "書類提出", "担当": a, "社用車": "軽トラ②", "状態": "予定", "通知": getNoticeText(today), "電話対応": "", "備考": "サンプル"},
  ...
  appendObjectsToSheet(ss.getSheetByName("車検管理"), [
    {"車両名": "サンプル1号車", "車番": "秋田000あ0001", "車検期限": addDaysForSample_(today, 20), "次回車検期限": addYearsForSample_(addDaysForSample_(today, 20), 1), "通知": getNoticeText(addDaysForSample_(today, 20)), "保険期限": addDaysForSample_(today, 20), "次回保険期限": addYearsForSample_(addDaysForSample_(today, 20), 1), "状態": "予約済", "写真": "", "備考": "車検テスト用"},
    {"車両名": "サンプル2号車", "車番": "秋田000あ0002", "車検期限": addDaysForSample_(today, -5), "次回車検期限": addYearsForSample_(today, 1), "通知": getNoticeText(addDaysForSample_(today, -5)), "保険期限": "", "次回保険期限": "", "状態": "未対応", "写真": "", "備考": "保険なし車両テスト用"}
  ]);
...
function formatSheetBase_(sheet) {
  ...
  if (maxRows >= 2) {
    sheet.setRowHeights(2, maxRows - 1, 38);
  }
}
