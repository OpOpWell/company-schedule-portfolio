/**
 * 社内共有管理 v7.1 GitHub公開用サンプル版
 *
 * Google Sheets + Google Apps Script による社内スケジュール管理システムです。
 *
 * - 担当者15人と既読確認者3人を分離
 * - 一覧スケジュール・要確認一覧を自動生成
 * - Googleカレンダー連携
 ...
 * - 既読率集計
 *
 * GitHub公開用のため、社員名・地名・車両名・取引先名はすべて架空のサンプルです。
 */
...
function groupRowsByMonth() {
  ...
  if (!sheet) {
    SpreadsheetApp.getActiveSpreadsheet().toast("月ごとに折りたたむ対象シートがありません");
    return;
  }

  groupRowsByMonthForSheet(sheet);
  SpreadsheetApp.getActiveSpreadsheet().toast("現在のシートを月ごとに折りたたみました");
}
...
// GitHub公開用では、動作確認のため addSampleData() を追加しています。

/****************************************************************
 * GitHub公開用サンプルデータ
 * 実在社員名・実在地名・実在車両名・取引先名は使用していません。
 ****************************************************************/
...
function addSampleData() {
  ...
  // 先に createCompanySheets() を実行して、各シートを作成してください。
  // シートが存在しない場合は、サンプルデータは投入されません。
  ...
  writeObjectsToSheet(ss.getSheetByName("出先予定"), [
    ...
    },
    {
      ...
      "社用車": "社用車B",
      "状態": "移動中",
      "通知": getNoticeText(addDays(today, 1)),
      "電話対応": "",
      "既読": false,
      "担当者A": false,
      "担当者B": false,
  ...
  writeObjectsToSheet(ss.getSheetByName("車検管理"), [
    {
      "車両名": "社用車A",
      "車番": "サンプル-001",
      "車検期限": addDays(today, 30),
      "通知": getNoticeText(addDays(today, 30)),
      "保険期限": addDays(today, 60),
      "状態": "予定",
      "写真": "",
      "既読": false,
      "担当者A": false,
    ...
    },
    {
      "車両名": "社用車B",
      "車番": "サンプル-002",
      "車検期限": addDays(today, 3),
      "通知": getNoticeText(addDays(today, 3)),
      "保険期限": addDays(today, 45),
      "状態": "要確認",
      "写真": "",
      "既読": false,
      "担当者A": false,
  ...
  writeObjectsToSheet(ss.getSheetByName("行事予定"), [
    {
      ...
      "担当": "担当者D",
      "状態": "予定",
      "通知": getNoticeText(addDays(today, 7)),
      "既読": false,
      "担当者A": false,
      "担当者B": false,
      "担当者C": false
  ...
  refreshAll();
  SpreadsheetApp.getActiveSpreadsheet().toast("GitHub公開用サンプルデータを追加しました");
}
