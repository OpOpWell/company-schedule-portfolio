...
Google Workspace（Google Sheets / Google Apps Script / Google Calendar / Google Drive）と AppSheet を活用し、社内の予定・電話履歴・車検・日報・備品修理・免許資格などを一元管理する業務改善システムです。
...
3. スプレッドシートを再読み込みする
4. メニュー「社内管理」を表示する
5. `社内管理 → 本番用初期セットアップ（軽量）` を実行する
6. 設定シートに担当者・既読確認者を入力する
7. `社内管理 → 入力シートだけ設定反映（軽量）` を実行する
8. `社内管理 → 個人確認グループを作り直す` を実行する
9. `社内管理 → 個人確認列を縦書きにする` を実行する
10. `社内管理 → 全体更新（軽量）` を実行する
...
6. 既読列・個人確認列は Yes/No として扱う
7. 不要な裏方シートやSQL設計シートはAppSheet側で非表示にする
