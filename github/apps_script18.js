/**
 * 社内共有業務管理システム v9.6.11 安定版（社内管理説明シート統合版）
 *
 * 目的:
 * - v9.6.6〜v9.6.9の運用安定版をベースに、公開前にコードを整理。
 * - 設定シートの内部文言を整理。
 ...
 * 「運用ルール・トラブル対応・公開前確認」を説明するための概要シートです。
 */
function createCompanyManagementExplanationSheet() {
  ...
  const rows = [
    ...
    ["公開・共有", "プライバシー", "公開時は実名、会社名、地名、車番、取引先名、メールアドレス、電話番号を入れないでください。", "サンプル版では架空名・架空データを使います。"],
    ["公開・共有", "ポートフォリオ説明", "このシステムは、社内業務をGoogle Workspaceで一元管理する業務改善システムとして説明できます。", "単なる表ではなく、入力・集約・通知・既読・PDF・AppSheet連携まで含む構成です。"],
    ["公開・共有", "引き継ぎ", "手順シートと社内管理説明を残しておくことで、他の人でも運用手順を確認できます。", "本番では非表示にしても削除しない方がよいです。"]
  ];
