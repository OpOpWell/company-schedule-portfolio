/**
 * 社内共有管理 v7.1 GitHub公開用デモ版
 *
 * - デモ用担当者15人と既読確認者3人を分離
 * - 既読確認列はデモ用確認者3列のみ
 * - 担当プルダウンはデモ用担当者15人
...
/**
 * GitHub公開用メモ
 * - このファイル内の氏名・会社名・地名・車番はすべてデモ用の架空データです。
 * - 実運用する場合は STAFF_MEMBERS / PERSONAL_MEMBERS とサンプルデータを自社用に置き換えてください。
 */
...
function addSampleData() {
  ...
  writeObjectsToSheet(ss.getSheetByName("出先予定"), [
    {"日付": today, "行き先": "サンプルサンプル市役所", "用件": "書類提出", "担当": "担当者A", "社用車": "1号車", "状態": "予定", "通知": getNoticeText(today), "電話対応": ""},
    {"日付": addDays(today, 2), "行き先": "サンプル現場A", "用件": "工程確認", "担当": "担当者B", "社用車": "軽トラ", "状態": "進行中", "通知": getNoticeText(addDays(today, 2)), "電話対応": ""}
  ]);

  writeObjectsToSheet(ss.getSheetByName("車検管理"), [
    {"車両名": "1号車", "車番": "サンプル500 あ 0001", "車検日": addDays(today, 7), "通知": getNoticeText(addDays(today, 7)), "保険期限": addDays(today, 30), "状態": "予定", "写真": ""},
    {"車両名": "軽トラ", "車番": "サンプル480 い 0002", "車検日": addDays(today, -3), "通知": getNoticeText(addDays(today, -3)), "保険期限": addDays(today, 20), "状態": "未対応", "写真": ""}
  ]);

  writeObjectsToSheet(ss.getSheetByName("会議予定"), [
    {"日付": today, "会議名": "工程会議", "内容": "週間工程確認", "担当": "担当者C", "状態": "予定", "通知": getNoticeText(today), "資料": ""}
  ]);

  writeObjectsToSheet(ss.getSheetByName("行事予定"), [
    {"日付": addDays(today, 5), "行事名": "安全大会", "内容": "安全教育", "担当": "担当者A", "状態": "予定", "通知": getNoticeText(addDays(today, 5))}
  ]);

  writeObjectsToSheet(ss.getSheetByName("作業状況"), [
    {"現場": "サンプル現場A", "作業内容": "掘削作業", "状態": "施工中", "担当": "担当者B", "写真": "", "通知": "", "備考": "午前中に進捗確認"},
    {"現場": "サンプル現場B", "作業内容": "資材搬入", "状態": "予定", "担当": "担当者C", "写真": "", "通知": "", "備考": "午後搬入予定"}
  ]);

  writeObjectsToSheet(ss.getSheetByName("工事予定"), [
    {"工事名": "道路改良工事", "現場": "サンプル市中央地区", "開始日": today, "終了日": addDays(today, 30), "状態": "施工中", "担当": "担当者B", "通知": getNoticeText(today), "電話対応": "", "備考": "片側通行あり"},
    {"工事名": "水路補修工事", "現場": "サンプル市北地区", "開始日": addDays(today, -20), "終了日": addDays(today, -5), "状態": "完了", "担当": "担当者A", "通知": getNoticeText(addDays(today, -20)), "電話対応": "完了", "備考": "過去一覧移動テスト用"}
  ]);

  writeObjectsToSheet(ss.getSheetByName("電話履歴"), [
    {"日時": today, "相手": "サンプル建設", "内容": "資材納期確認", "電話対応": "折返し", "担当": "担当者C", "状態": "対応中", "通知": getNoticeText(today), "メモ": "午前中に再連絡"},
    {"日時": addDays(today, -10), "相手": "サンプル商事", "内容": "請求書確認", "電話対応": "完了", "担当": "担当者A", "状態": "完了", "通知": getNoticeText(addDays(today, -10)), "メモ": "過去一覧移動テスト用"}
  ]);

  writeObjectsToSheet(ss.getSheetByName("お知らせ"), [
    {"投稿日": today, "タイトル": "安全注意", "内容": "熱中症対策を徹底してください", "投稿者": "担当者A", "重要度": "高", "通知": "重要"},
    {"投稿日": addDays(today, 1), "タイトル": "社内連絡", "内容": "今週中に車両点検をお願いします", "投稿者": "担当者B", "重要度": "中", "通知": getNoticeText(addDays(today, 1))}
  ]);

  writeObjectsToSheet(ss.getSheetByName("個人ToDo"), [
    {"登録日": today, "担当": "担当者B", "内容": "見積書提出", "期限": addDays(today, 3), "状態": "未対応", "通知": getNoticeText(addDays(today, 3))},
    {"登録日": addDays(today, -15), "担当": "担当者C", "内容": "完了報告書作成", "期限": addDays(today, -2), "状態": "完了", "通知": getNoticeText(addDays(today, -2))}
  ]);

  writeObjectsToSheet(ss.getSheetByName("社用車予約"), [
    {"日付": today, "開始時刻": "09:00", "終了時刻": "12:00", "社用車": "1号車", "利用者": "担当者A", "行き先": "サンプル市役所", "用途": "書類提出", "状態": "予定", "通知": getNoticeText(today), "備考": ""},
    {"日付": today, "開始時刻": "13:00", "終了時刻": "17:00", "社用車": "軽トラ", "利用者": "担当者B", "行き先": "サンプル現場A", "用途": "現場確認", "状態": "予定", "通知": getNoticeText(today), "備考": ""},
    {"日付": addDays(today, -30), "開始時刻": "08:30", "終了時刻": "10:00", "社用車": "バン", "利用者": "担当者C", "行き先": "客先", "用途": "打合せ", "状態": "完了", "通知": getNoticeText(addDays(today, -30)), "備考": "過去一覧移動テスト用"}
  ]);

  // 先に通常シートから一覧・ダッシュボードを作る。
  refreshAll();

  // 過去一覧だけは refreshAll の後に入れる。これで空欄やグループだけ残る事故を防ぐ。
  addArchiveSampleData();
  groupArchiveByMonthWithHeader();

  SpreadsheetApp.getActiveSpreadsheet().toast("サンプルデータを投入しました");
}

function addArchiveSampleData() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName("過去一覧");
  if (!sheet) return;

  const headers = SHEET_HEADERS["過去一覧"];
  resetSheet(sheet, headers.length);
  setupSheet(sheet, headers);
  ...
  writeObjectsToSheet(sheet, [
    {"日付": addDays(today, -90), "種類": "工事予定", "内容": "河川改修工事 / サンプル川", "担当": "担当者A", "状態": "完了", "通知": "期限切れ", "電話対応": "", "元シート": "工事予定", "備考": "完了済", "移動日": addDays(today, -60)},
    {"日付": addDays(today, -60), "種類": "会議予定", "内容": "工程会議 / 月例", "担当": "担当者B", "状態": "完了", "通知": "期限切れ", "電話対応": "", "元シート": "会議予定", "備考": "", "移動日": addDays(today, -45)},
    {"日付": addDays(today, -45), "種類": "電話履歴", "内容": "サンプル建設 / 資材確認", "担当": "担当者C", "状態": "対応中", "通知": "期限切れ", "電話対応": "完了", "元シート": "電話履歴", "備考": "", "移動日": addDays(today, -40)},
    {"日付": addDays(today, -30), "種類": "出先予定", "内容": "サンプル市役所 / 書類提出", "担当": "担当者A", "状態": "完了", "通知": "期限切れ", "電話対応": "", "元シート": "出先予定", "備考": "", "移動日": addDays(today, -25)},
    {"日付": addDays(today, -15), "種類": "行事予定", "内容": "安全大会", "担当": "担当者B", "状態": "完了", "通知": "期限切れ", "電話対応": "", "元シート": "行事予定", "備考": "", "移動日": addDays(today, -10)}
  ]);
}
