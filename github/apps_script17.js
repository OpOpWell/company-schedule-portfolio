/**
 * 社内共有業務管理システム v9.6.7 安定版（v9.6.6ベース・フィルタ安全化）
 *
 * - 担当者15人と既読確認者3人を分離
 * - 既読確認列は山田・高橋・鈴木の3列のみ
 * - 担当プルダウンは山田を先頭にした15人
 ...
 * - v9.6.7: v9.6.6の運用安定版をベースに、フィルタ二重作成エラーだけを安全化
 ...
 *
 * v9.6.2追加
 * - 設定シートの内部メモ文言を公開・実運用どちらでも自然な表現へ変更
 * - 帳簿出力設定のTRUE/FALSEをチェックボックス表示へ正規化
 * - 電話履歴一覧の出力列を「状態」ではなく「対応メモ」へ修正
 * - 着工前・施工中・要確認などの状態カラーを意味別に再整理
 * - 手順シートを追加し、本番・テスト・日常・月次の操作手順を明確化
 * - 設定反映時の入力規則クリア範囲を必要範囲に抑えて軽量化
 *
 * v9.6.3追加
 * - Google Sheetsのタイムアウト対策として、初期セットアップを段階別に分割
 * - 本番用初期セットアップ（軽量）では、重い全シート設定反映を行わず主要入力シート中心に限定
 * - 入力規則の既定範囲を1000行から300行へ軽量化し、必要に応じて自動拡張
 * - 設定反映を「入力シートのみ」「出力・裏方シートのみ」「全シート」に分離
 * - テスト環境セットアップも重い一括処理を避け、段階別実行を推奨する構成へ変更
 * - 条件付き書式・チェックボックス・グループ作成は必要時に個別実行できるようメニューを整理

 *
 * v9.6.4追加
 * - onEditでは一覧スケジュール・要確認一覧・集計を再作成しない軽量仕様へ変更
 * - 入力後は手動の「全体更新（軽量）」でまとめて反映
 * - 全体更新時に主要入力シートを日時順・期限順に並び替えてから集約
 * - 車検管理・電話履歴・日報・過去一覧・帳簿PDF履歴などを用途別に昇順/降順ソート
 * - 状態・通知・種類の条件付き書式を意味ごとに再整理
 * - 追加行の入力規則再設定を軽量化
 */
 ...
 * - サンプル版では架空名を使用しています。実運用時は「設定」シートの値を変更してください。
 */
...
function normalizeSettingsSheetNotes_(sheet) {
  ...
  const updated = values.map(row => {
    ...
    } else if (currentNote.indexOf("GitHub") >= 0 || currentNote.indexOf("差し替え") >= 0) {
...
function createProcedureSheet() {
  ...
  const rows = [
    ...
    ["公開前確認", 1, "匿名化確認", "push前", "実名・社名・地名・車番・取引先名が入っていないか確認します。"]
  ];
...
function createProcedureSheet() {
  ...
  const rows = [
    ...
    ["公開前確認", "1", "匿名化確認", "push前", "実名・社名・地名・車番・取引先名が残っていないか確認"],
...
function formatInputSheetsForLongText() {
  ...
  SpreadsheetApp.getActiveSpreadsheet().toast('入力用・確認用シートの表示幅を調整しました（個人確認列は縦書き）');
}
...
function formatPersonalCheckColumnsVertical_(sheet) {
  ...
  if (readCol > 0) {
    try {
      sheet.setColumnWidth(readCol, 54);
      sheet.getRange(1, readCol, sheet.getMaxRows(), 1)
        .setHorizontalAlignment('center')
        .setVerticalAlignment('middle')
        .setWrap(true);
      sheet.getRange(1, readCol).setVerticalText(false);
    } catch (e) {}
  }
  ...
  personalMembers.forEach(member => {
    ...
    try {
      sheet.setColumnWidth(col, 38);

      // ヘッダーだけ縦書き。チェックボックス部分は中央揃え。
